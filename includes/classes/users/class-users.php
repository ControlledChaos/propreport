...
class Users {
	...
	public function __construct() {
		 ...
		 *
		 * If `PRP_ALLOW_ADMIN_COLOR_PICKER` is set to false.
		 * This can be defined in the system config file.
		 */
		if ( defined( 'PRP_ALLOW_ADMIN_COLOR_PICKER' ) && false == PRP_ALLOW_ADMIN_COLOR_PICKER ) {
	...
	public function admin_enqueue_scripts() {
		...
		if ( 'profile.php' == $pagenow || 'user-edit.php' == $pagenow ) {
			wp_enqueue_script(
				...
				PRP_URL . 'assets/js/user-bio' . $suffix . '.js',
				[ 'jquery' ],
				false,
				true
			);
	...
	public function profile_editor( $user ) {
		...
		ob_start();
		?>
		<h2><?php _e( 'User Details', 'propreport' ); ?></h2>

		<table class="form-table">
			<tbody>
				<tr>
					<th><label for="description"><?php _e( 'Biographical Info', 'propreport' ); ?></label></th>
					<td>
						<?php
						...
						?>
						<p class="description"><?php _e( 'Share a little biographical information to fill out your profile. This may be shown publicly.', 'propreport' ); ?></p>
					</td>
				</tr>
			</tbody>
		</table>
