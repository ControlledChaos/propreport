...
class User_Roles_Caps {
	...
	public function get_roles() {
		...
		return apply_filters( 'prp_get_roles', $wp_roles->role_names );
	}
	...
	public function get_editable_roles() {
		...
		return apply_filters( 'prp_get_editable_roles', (array) $final_roles );
	}
	...
	public function get_user_roles( $user = 0 ) {
		...
		return apply_filters( 'prp_get_user_roles', $roles );
	}
	...
	public function update_roles( $user_id = 0, $roles = [] ) {
		...
		do_action( 'prp_before_update_roles', $user_id, $roles );
		...
		do_action( 'prp_after_update_roles', $user_id, $roles, $user->roles );
	...
	public function can_update_roles() {
		...
		do_action( 'prp_before_can_update_roles' );
	...
	public function profile_role_output_checklist( $user ) {
		...
		wp_nonce_field( 'update-prp-multiple-roles', 'prp_multiple_roles_nonce' );
		...
		include( apply_filters( 'prp_checklist_template', PRP_PATH . 'views/backend/forms/user-roles-checklist.php' ) );
	...
	public function profile_role_process_checklist( $user_id ) {
		 ...
		 * `prp_multiple_roles_nonce` is available, else bail.
		 * If we continue to process and update_roles(),
		 * all user roles will be lost. We check for
		 * `prp_multiple_roles_nonce` rather than `prp_multiple_roles`
		 * as this input/variable will be empty if all role inputs
		 * are left unchecked.
		 */
		if ( ! isset( $_POST['prp_multiple_roles_nonce'] ) ) {
			return;
		}

		if ( ! wp_verify_nonce( $_POST['prp_multiple_roles_nonce'], 'update-prp-multiple-roles' ) ) {
			return;
		}
		...
		if ( isset( $_POST['prp_multiple_roles'] ) && is_array( $_POST['prp_multiple_roles'] ) ) {
			$new_roles = $_POST['prp_multiple_roles'];
		} else {
			$new_roles = [];
		}
	...
	public function network_add_roles_in_signup_meta( $meta, $domain, $path, $title, $user, $user_email, $key ) {
		...
		if ( isset( $_POST['prp_multiple_roles_nonce'] ) && ! wp_verify_nonce( $_POST['prp_multiple_roles_nonce'], 'update-prp-multiple-roles' ) ) {
			return;
		}
	...
	public function network_add_roles_after_activation( $user_id, $password, $meta ) {
		...
		if ( ! empty( $meta['prp_roles'] ) ) {
			$this->update_roles( $user_id, $meta['prp_roles'] );
		}
	}
	...
	public function list_role_column_replace( $columns ) {
		...
		unset( $columns['role'] );
		$columns['prp_multiple_roles_column'] = __( 'Roles', 'propreport' );
	...
	public function list_role_column_content( $output, $column, $user_id ) {
		...
		if ( 'prp_multiple_roles_column' !== $column ) {
			return $output;
		}
		...
		ob_start();
		include( apply_filters( 'prp_column_template', PRP_PATH . 'views/backend/forms/user-roles-admin-column.php' ) );
		return ob_get_clean();
	}
}
